...
> 这份文档每周一填本周目标,周末助手做评估。Codex 进入项目自动读取,等于持久化记忆。
...
- **内核**:复刻并精简 Codex 架构的 ReAct agent(工具调用 + 记忆 + RAG)
...
### 共同开发与学习准则(D5.4 起长期执行)

- Codex 可以负责跨模块、结构复杂或容易引入回归的主体实现,但必须先讲清目的、输入输出、数据流和伪代码。
- 每个功能都要留出范围合适的小函数、测试用例或局部修改让用户亲自完成,随后由 Codex 检查正确性和解释问题。
- 目标不是背诵代码或强行独立写出所有复杂实现,而是逐步能够对项目的正确性负责。
- "看懂"不能只指看懂注释。至少要逐步做到:能脱离代码讲清数据流、能预测测试输出、能完成小改动、能根据测试和 traceback 定位问题。
- Codex 检查用户代码时应直接指出逻辑、语法和工程问题,不能为了维持进度直接代写并跳过用户尚未消化的部分。
- 随能力提升逐步扩大用户亲自实现的范围:先写简单分支和测试,再写单函数,最后完成小型跨模块功能。
...
>
> 净有效工作日 ~15–18 天 vs 原计划 7 天,完成概率 **70–80%**(原 AGENTS.md 写 85% 在新时间线下基本合理)。
...
> - B2 快是因为 LeetCode 基础覆盖了 dict/list，AGENTS.md 原先"接近零基础"低估了
> - B3 密集迭代 4–5 轮改对，没卡壳
